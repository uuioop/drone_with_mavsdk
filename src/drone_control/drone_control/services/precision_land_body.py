...
class PrecisionLand:
    ...
    async def update(self):
        ...
        # --- 状态机逻辑 ---
        if self.state == PrecisionLandState.SEARCH:
            await self.mavsdk_controller.set_velocity_body(0.0, 0.0, 0.0, 0.0)
            # 优先寻找前置引导标记
            if is_front_valid:
            ...
            vy = np.clip(0.2 * error_y, -0.2, 0.2)
            vz = np.clip(0.3 * error_z, -0.3, 0.3)
            self.logger.info(f"[PL] X方向误差: {error_x}, Y方向误差: {error_y}, Z方向误差: {error_z}")
            # 安全检查：如果离引导标记太近但仍未看到平台标记，则进入悬停
            if abs(error_x) < 0.5: # 如果X方向距离小于0.5米
